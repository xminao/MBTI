...
type (
	...
	StudentInfo struct {
		...
		StudentName sql.NullString `db:"student_name"` // 姓名
...
func (m *defaultStudentInfoModel) Insert(ctx context.Context, data *StudentInfo) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("insert into %s (%s) values ($1, $2, $3)", m.table, studentInfoRowsExpectAutoSet)
		return conn.ExecCtx(ctx, query, data.StudentId, data.StudentName, data.ClassId)
